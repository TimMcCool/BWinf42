# Erweiterung 1: Der Hof ist nun kein geschlossenes System mehr, Laub kann den Hof verlassen.
# Die Hof.blase() Funktion in hof.py wurde modifiziert.
# Um die den Hof verlassende Menge an Laub zu verringern wurde außerdem die Lösungsstrategie angepasst.
...
Q = (4,4) # Index von Feld Q festlegen
hof_size = (10,10) # Hofseitenlängen festlegen
...
tolerated_amount = 2 # Blattmenge, die auf nicht vollständig leerbaren Feldern als vernachlässigbar gilt
...
class Solver2:
    ...
    def clear_edges(self):
        """
        1.Phase: Ränder (bis auf oberen Rand) entlauben, um zu vermeiden, dass beim Leeren der Ecken Laub verloren geht
        """
        ...
        if self.hof.x_size > 4:
            self.add_operation(self.edge_to_mid((2, self.hof.y_size-1), (2, self.hof.y_size-2)))
        elif self.hof.x_size > 3:
            self.add_operation(self.edge_to_mid((1, self.hof.y_size-1), (1, self.hof.y_size-2)))
        ...
        for start_y in range(self.hof.y_size-1,3,-1):
            self.add_operation(dict(feld0=(0,start_y),blow_direction=(0,-1))) # Nicht-Eckfelder am unteren Rand leeren
        for start_y in range(self.hof.y_size-1,3,-1):
            self.add_operation(dict(feld0=(self.hof.x_size-1,start_y),blow_direction=(0,-1))) # Nicht-Eckfelder am unteren Rand leeren
        if self.hof.y_size > 4:
            self.add_operation(self.edge_to_mid((self.hof.x_size-1, 2), (self.hof.x_size-2, 2)))
            self.add_operation(self.edge_to_mid((0, 2), (1, 2)))
        elif self.hof.y_size > 3:
            self.add_operation(self.edge_to_mid((self.hof.x_size-1, 1), (self.hof.x_size-2, 1)))
            self.add_operation(self.edge_to_mid((0, 1), (1, 1)))
        for start_x in range(0, self.Q[0]-1):
            for start_y in range(self.hof.y_size-3,0,-1):
                if start_x == self.Q[0]-2 and self.hof.does_exist((start_x-1, start_y)):
                    self.add_operation(Muster(self, [(start_x+1, start_y)], [dict(feld0=(start_x-1, start_y), blow_direction=(1,0))], self.tolerated_amount))
                else:
                    self.add_operation(dict(feld0=(start_x,start_y), blow_direction=(1,0)))
        ...
        if self.hof.does_exist((Q[0]-3,self.hof.y_size-2)):
            self.add_operation(Muster(self, [(Q[0]-1, self.hof.y_size-2)], [dict(feld0=(Q[0]-3, self.hof.y_size-2), blow_direction=(1,0))], self.tolerated_amount))
        if self.hof.does_exist((Q[0]+3,self.hof.y_size-2)):
            self.add_operation(Muster(self, [(Q[0]+1, self.hof.y_size-2)], [dict(feld0=(Q[0]+3, self.hof.y_size-2), blow_direction=(-1,0))], self.tolerated_amount))
            ...
            for start_x in range(self.hof.x_size-1,-1,-1):
                if not (start_x == self.Q[0] and (start_y <= self.Q[1]+1 or self.hof.y_size == 5)): # Feld Q und Felder über Q nicht unnötigerweise leeren, da hier nachher das Laub sowieder wieder hintransportiert werden muss
                    ...
                    self.add_operation(Muster(
                        self, [(start_x, start_y-1)], [dict(feld0=(start_x, start_y), blow_direction=(0,-1))], self.tolerated_amount
                    ))
        ...
        if Q == (Q[0],1):
            if not self.hof.is_edge((Q[0],2)):
                if self.hof.does_exist((Q[0],4)):
                    self.add_operation(Muster(self, [(Q[0],2)], [dict(feld0=(Q[0],4), blow_direction=(0,-1))], self.tolerated_amount))
                else:
                    self.add_operation(dict(feld0=(Q[0],3), blow_direction=(0,-1)))
        else:
    ...
    def build_strategy(self, *, Q):
        ...
        self.Q = Q
        # Strategie speichern
        self.strategy.append("\n[Phase 1: Ränder (bis auf oberen Rand) entlauben]")
        self.clear_edges()
...
h = Hof(hof_size, Rules(
    ...
    A_seitenabtrieb = 0.1, B_vorne_abtrieb = 0.1, A_noB_seitenabtrieb=0.5, wall_resistance=0.5, # Hier können die Blasregeln angepasst werden
...
print("Anteil der Blätter auf Q an der ursprünglichen Gesamtlaubmenge:", s.hof.felder[s.Q]*100/(s.hof.felder.size*s.hof.startwert), "%")
print("Gesamtlaubmenge, die sich noch auf dem Hof befindet:", np.sum(s.hof.felder), "von urspr.", s.hof.felder.size*s.hof.startwert)
