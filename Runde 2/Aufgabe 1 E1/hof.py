...
# Erweiterung: Der Hof ist nun kein geschlossenes System mehr, Laub kann den Hof verlassen.
...
class Rules:
    ...
    def __init__(self, *, wall_resistance=0.5, A_seitenabtrieb = 0.1, B_vorne_abtrieb = 0.1, A_noB_seitenabtrieb=0.5*0.95, use_binomial=True, binomial_rank="random", binomial_handle_ties="higher"):
        """
        Args:
    ...
    def blase(self, feld0, blow_direction):
        ...
        if self.does_exist(feldB):
            ...
            if self.does_exist((feldB[0]+blow_direction[0], feldB[1]+blow_direction[1])):
                self.felder[(feldB[0]+blow_direction[0], feldB[1]+blow_direction[1])] += B_vorne_abtrieb
            else:
                if self.rules.use_binomial:
                    remain_on_B, _ = binomial_likeliest(n=B_vorne_abtrieb, p=self.rules.wall_resistance, rank=self.rules.binomial_rank, handle_ties=self.rules.binomial_handle_ties)
                    new_feldB_value += remain_on_B
                else:
                    new_feldB_value += B_vorne_abtrieb * self.rules.wall_resistance
            ...
            if self.does_exist((feldB[0]+orthogonal_direction[0], feldB[1]+orthogonal_direction[1])):
                self.felder[(feldB[0]+orthogonal_direction[0], feldB[1]+orthogonal_direction[1])] += A_seitenabtrieb_1
            else:
                if self.rules.use_binomial:
                    remain_on_B, _ = binomial_likeliest(n=A_seitenabtrieb_1, p=self.rules.wall_resistance, rank=self.rules.binomial_rank, handle_ties=self.rules.binomial_handle_ties)
                    new_feldB_value += remain_on_B
                else:
                    new_feldB_value += A_seitenabtrieb_1 * self.rules.wall_resistance
            if self.does_exist((feldB[0]-orthogonal_direction[0], feldB[1]-orthogonal_direction[1])):
                self.felder[(feldB[0]-orthogonal_direction[0], feldB[1]-orthogonal_direction[1])] += A_seitenabtrieb_2
            else:
                if self.rules.use_binomial:
                    remain_on_B, _ = binomial_likeliest(n=A_seitenabtrieb_2, p=self.rules.wall_resistance, rank=self.rules.binomial_rank, handle_ties=self.rules.binomial_handle_ties)
                    new_feldB_value += remain_on_B
                else:
                    new_feldB_value += A_seitenabtrieb_2 * self.rules.wall_resistance
            ...
            # -> Es gibt kein Feld B.
            if self.rules.use_binomial:
                leave_hof, _ = binomial_likeliest(n=self.felder[feldA], p=(1-self.rules.wall_resistance), rank=self.rules.binomial_rank, handle_ties=self.rules.binomial_handle_ties)
                A_noB_seitenabtrieb_1, _ = binomial_likeliest(n=self.felder[feldA]-leave_hof, p=self.rules.A_noB_seitenabtrieb, rank=self.rules.binomial_rank, handle_ties=self.rules.binomial_handle_ties)
                A_noB_seitenabtrieb_2 = self.felder[feldA]-leave_hof-A_noB_seitenabtrieb_1
            else:
                leave_hof = self.felder[feldA] * (1-self.rules.wall_resistance)
                A_noB_seitenabtrieb_1 = (self.felder[feldA] - leave_hof) * self.rules.A_noB_seitenabtrieb
                A_noB_seitenabtrieb_2 = A_noB_seitenabtrieb_1
            new_feldA_value = self.felder[feldA] - (A_noB_seitenabtrieb_1 + A_noB_seitenabtrieb_2) - leave_hof
            ...
            if self.does_exist((feldA[0]+orthogonal_direction[0], feldA[1]+orthogonal_direction[1])):
                self.felder[(feldA[0]+orthogonal_direction[0], feldA[1]+orthogonal_direction[1])] += A_noB_seitenabtrieb_1
            else:
                # -> Feld A ist ein Eckfeld, ein Teil des Laubs verbleibt also auf Feld A.
                if self.rules.use_binomial:
                    new_feldA_value += binomial_likeliest(n=A_noB_seitenabtrieb_1, p=self.rules.wall_resistance, rank=self.rules.binomial_rank, handle_ties=self.rules.binomial_handle_ties)[0]
                else:
                    new_feldA_value += A_noB_seitenabtrieb_1 * self.rules.wall_resistance
            if self.does_exist((feldA[0]-orthogonal_direction[0], feldA[1]-orthogonal_direction[1])):
                self.felder[(feldA[0]-orthogonal_direction[0], feldA[1]-orthogonal_direction[1])] += A_noB_seitenabtrieb_2
            else:
                # -> Feld A ist ein Eckfeld, ein Teil des Laubs verbleibt also auf Feld A.
                if self.rules.use_binomial:
                    new_feldA_value += binomial_likeliest(n=A_noB_seitenabtrieb_2, p=self.rules.wall_resistance, rank=self.rules.binomial_rank, handle_ties=self.rules.binomial_handle_ties)[0]
                else:
                    new_feldA_value += A_noB_seitenabtrieb_2 * self.rules.wall_resistance
