# Ansatz 2
# Deutlich besserer Ansatz als aufgabe1_solver1.py (siehe Dokumentation). Dieser soll bewertet werden.
...
Q = (6,6) # Index von Feld Q festlegen
hof_size = (20,20) # Hofseitenlängen festlegen
...
class Solver2:
    ...
    def edge_to_mid(self, source_edge_field, target_field) -> Muster:
        ...
        if (not self.hof.is_edge(source_edge_field)) or self.hof.is_edge(target_field):
            return
        orthogonal_direction = (source_edge_field[0]-target_field[0], source_edge_field[1]-target_field[1]) # Die Richtung, die zum Rand hinzeigt
    ...
    def move_to_top_line(self):
        ...
        for start_y in range(self.hof.y_size-1,1,-1):
            for start_x in range(self.hof.x_size-1,-1,-1):
                if not (start_x == self.Q[0] and (start_y <= self.Q[1]+1 or self.hof.y_size == 5) and (self.Q[1] != 1)): # Feld Q und Felder über Q nicht unnötigerweise leeren, da hier nachher das Laub sowieder wieder hintransportiert werden muss
...
print("Anteil der Blätter auf Q an der Gesamtlaubmenge:", s.hof.felder[s.Q]*100/np.sum(s.hof.felder), "%")
