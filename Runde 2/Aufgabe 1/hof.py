...
class Hof:
    ...
    def __init__(self, size, rules = Rules(), *, startwert=100, felder=None, blas_counter=0):
        ...
        if felder is None:
            ...
            self.felder = np.array(felder, dtype=int if self.rules.use_binomial else float)
    ...
    def blase(self, feld0, blow_direction):
        ...
        if self.does_exist(feldB):
            ...
            if self.does_exist((feldB[0]+blow_direction[0], feldB[1]+blow_direction[1])):
                ...
                if self.rules.use_binomial:
                    B_seitenabtrieb_1, _ = binomial_likeliest(n=B_vorne_abtrieb, p=(1-self.rules.s1)/2, rank=self.rules.binomial_rank, handle_ties=self.rules.binomial_handle_ties) # Anzahl an Blättern von Feld B, die normalerweise auf dem Feld vor B gelandet wären, nun aber auf dem einen Feld neben Feld B landen (Seitenabtrieb 1)
                    B_seitenabtrieb_2, _ = binomial_likeliest(n=B_vorne_abtrieb-B_seitenabtrieb_1, p=((1-self.rules.s1)/2)/(1-(1-self.rules.s1)/2), rank=self.rules.binomial_rank, handle_ties=self.rules.binomial_handle_ties) # Anzahl an Blättern von Feld B, die normalerweise auf dem Feld vor B gelandet wären, nun aber auf dem anderen Feld neben Feld B landen (Seitenabtrieb 2)
                    new_feldB_value += B_vorne_abtrieb - B_seitenabtrieb_1 - B_seitenabtrieb_2
                else:
                    new_feldB_value += B_vorne_abtrieb * self.rules.s1
                    B_seitenabtrieb_1 = B_vorne_abtrieb * (1-self.rules.s1) * 0.5
            ...
            if self.does_exist((feldB[0]+orthogonal_direction[0], feldB[1]+orthogonal_direction[1])):
                self.felder[(feldB[0]+orthogonal_direction[0], feldB[1]+orthogonal_direction[1])] += A_seitenabtrieb_1 + B_seitenabtrieb_1
            else:
                new_feldB_value += A_seitenabtrieb_1 + B_seitenabtrieb_1
            if self.does_exist((feldB[0]-orthogonal_direction[0], feldB[1]-orthogonal_direction[1])):
                self.felder[(feldB[0]-orthogonal_direction[0], feldB[1]-orthogonal_direction[1])] += A_seitenabtrieb_2 + B_seitenabtrieb_2
            else:
